    ...
    module.exports = {
        ...
        "unpack": function(buffer, offset) {
            ...
            if (nw_src_mask < 32) {
                ...
                match.nw_src_raw = buffer.readUInt32BE(offset + offsets.nw_src, true);
            ...
            if (nw_dst_mask < 32) {
                ...
                match.nw_dst_raw = buffer.readUInt32BE(offset + offsets.nw_dst, true);
        ...
        "pack": function(match, buffer, offset) {
            ...
            // TODO validate
            if ('in_port' in match) {
                if (match.in_port == 'OFPP_LOCAL') {
                    var in_port = ofp.ofp_port_no.OFPP_LOCAL;
                } else {
                    var in_port = match.in_port;
                }
            } else {
              wildcards |= ofp.ofp_flow_wildcards.OFPFW_IN_PORT;
              var in_port = 0;
            }
            ...
            if ('dl_src' in match) {
                packet.stringToEth(match.dl_src, buffer, offset + offsets.dl_src);
            } else {
              wildcards |= ofp.ofp_flow_wildcards.OFPFW_DL_SRC;
            }
           

            if ('dl_dst' in match) {
                packet.stringToEth(match.dl_dst, buffer, offset + offsets.dl_dst);
            } else {
              wildcards |= ofp.ofp_flow_wildcards.OFPFW_DL_DST;
            }
            ...
            if ('dl_vlan_pcp' in match) {
                var dl_vlan_pcp = match.dl_vlan_pcp;
            } else {
                var dl_vlan_pcp = 0;
            ...
            if ('nw_src' in match) {
                if (typeof(match.nw_src) === 'string') {
                    packet.stringToIPv4(match.nw_src, buffer, offset + offsets.nw_src);
                } else {
                    buffer.writeUInt32BE(match.nw_src, offset + offsets.nw_src, true);
                }
            } else {
                ...
                wildcards |= ofp.OFPFW_NW_SRC_ALL;
            }
           

            if ('nw_dst' in match) {
                if (typeof(match.nw_dst) === 'string') {
                    packet.stringToIPv4(match.nw_dst, buffer, offset + offsets.nw_dst);
                } else {
                    buffer.writeUInt32BE(match.nw_dst, offset + offsets.nw_dst, true);
                }
            } else {
                ...
                wildcards |= ofp.ofp_flow_wildcards.OFPFW_NW_DST_ALL;
            }

            if ('tp_src' in match) {
                var tp_src = match.tp_src;
            } else {
                var tp_src = 0;
