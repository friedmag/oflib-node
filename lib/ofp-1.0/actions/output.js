...
module.exports = {
  "unpack" : function(buffer, offset) {
    ...
    /* special values */
    if (action.body.port in ofp.ofp_port) {
      action.body.port = ofp.ofp_port[action.body.port];
      return {
        "action" : action,
        "offset" : offset + len
      }
  ...
  "pack" : function(action, buffer, offset) {
    ...
    // TODO: why isn't this ofp.ofp_port same as below?
    if (action.body.port == 'OFPP_CONTROLLER') {
      buffer.writeUInt16BE(ofp.ofp_port.OFPP_CONTROLLER, offset + offsets.port, true);
    ...
    /* special values */
    if (action.body.port in ofp.ofp_port) {  //TODO: why is this ofp_port_no up a level?
